...
export async function searchHandler(args: unknown): Promise<SearchResponse> {
	...
	if (
		scrapingResults.status === 'fulfilled' &&
		scrapingResults.value.length > 0
	) {
		successfulResults.push(scrapingResults.value);
		console.log(
			`Scraping search returned ${scrapingResults.value.length} results`
		);
	} else {
		console.error(
			'Scraping search failed:',
			scrapingResults.status === 'rejected'
				? scrapingResults.reason
				: 'no results'
		);
	}
	...
	const sources = successfulResults.map((_, i) =>
		i === 0 ? 'api' : ('scraping' as const)
	);
